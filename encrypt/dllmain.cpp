...
void* memcpy(void* dest,const void* src,int count)
{
      ...
      char* dest2=(char*)dest; const char* src2=(const char*)src;
      for(int i=0;i<count;i++) dest2[i]=src2[i];
      return dest2;
}
void memset(void* dest,char ch,int count)
{
     char* p=(char*)dest;
     for(int i=0;i<count;i++) p[i]=ch;
}
void intToBytes(unsigned int value,char* buffer)
{
    //value=abs($value);
    for (int i=0;i<4;i++)
	{
...
/*
void byteCopy(char* dest, char* src, int length)
{
    for(;length>0;length--)
    {
               *(dest++)=*(src++);   
...
*/
/*
unsigned int __crc32_reflect(unsigned int ref,unsigned int ch) {        // Reflects CRC bits in the lookup table
...
void __crc32_init_table() {            // Builds lookup table array
        ...
        for(i=0;i <= 0xFF;++i) {
            ...
            for(j=0;j < 8;++j) {
                __crc32_table[i]=((__crc32_table[i] << 1) ^
                    ((__crc32_table[i] & (1 << 31))?polynomial:0));
            }
            __crc32_table[i] = __crc32_reflect(__crc32_table[i], 32);
        }
    }
*/
unsigned int __crc32_string(const char * text,int length) {        // Creates a CRC from a text string
        // Once the lookup table has been filled in by the two functions above,
        // this function creates all CRCs using only the lookup table.

        // You need variables because negative values
        // introduce high bits where zero bits are required.
        // PHP doesn't have integers:
...
/* 
extern "C" DLLIMPORT __stdcall bool CRC64(char* bufferIn,int inLength,char* bufferOut,int outLength)
{
    if (bufferIn==0 || bufferOut==0) return false;
    if (inLength<1 || outLength<8) return false;
    __crc32_init_table();
    char* p1=new char[4+inLength];
    char* p2=new char[4];
    intToBytes(__crc32_string(bufferIn,inLength),p2);     
    byteCopy(p1,p2,4);
    byteCopy(p1+4,bufferIn,inLength);
    byteCopy(bufferOut,p2,4);
    intToBytes(__crc32_string(p1,4+inLength),p2);
    
    //char temp[128];
    //sprintf(temp,"%d",__crc32_reflect(44,5));
    //MessageBox(0,temp,"Message",0);
    //
    byteCopy(bufferOut+4,p2,4);
    delete p1,p2;
    return true;
}
*/
extern "C" DLLIMPORT __stdcall bool CRC32(char* bufferIn,int inLength,char* bufferOut,int outLength)
{
    if (bufferIn==0 || bufferOut==0) return false;
    if (outLength<4) return false;
    //__crc32_init_table();
    intToBytes(__crc32_string(bufferIn,inLength),bufferOut);
    return true;
}
...
extern "C" DLLIMPORT __stdcall bool SHA1(char* bufferIn,int inLength,char* bufferOut,int outLength)
{
    if (bufferIn==0 || bufferOut==0) return false;
    if (inLength<1 || outLength<20) return false;
    SHA1Context sha;
    SHA1Reset(&sha);
    SHA1Input(&sha, (unsigned char*)bufferIn, inLength);
    if (!SHA1Result(&sha)) return false;
    else
    {
        for(int i=0;i<=4;i++) 
        {               
                bufferOut[i*4]=(sha.Message_Digest[i] & 0xFF000000)>>24;
                bufferOut[i*4+1]=(sha.Message_Digest[i] & 0xFF0000)>>16;
                bufferOut[i*4+2]=(sha.Message_Digest[i] & 0xFF00)>>8;                
                bufferOut[i*4+3]=(sha.Message_Digest[i] & 0xFF);
        }
    }
    return true;
...
/*Function F*/
unsigned int Blowfish_F(unsigned int auiS[][MAX_SBLOCK_YSIZE], unsigned int ui)
{
...
extern "C" DLLIMPORT __stdcall bool BlowFish(char* bufferIn,int inLength,char* key,char* bufferOut,int outLength,int mode)
{
